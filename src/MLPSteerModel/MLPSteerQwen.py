...
class MLPSteerQwen2DecoderLayer(Qwen2DecoderLayer):
...
class MLPSteerQwen2Model(Qwen2Model):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [MLPSteerQwen2DecoderLayer(
                config=config, 
                layer_idx=layer_idx, 
            )
...
class MLPSteerQwen2ForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.model = MLPSteerQwen2Model(config=config)
